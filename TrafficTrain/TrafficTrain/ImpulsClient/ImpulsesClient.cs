...
using TrafficTrain.Impulsesver.Client.requests;
using TrafficTrain;
...
namespace TrafficTrain.Impulsesver.Client
{
    ...
    {
        static readonly ILog log = LogManager.GetLogger(typeof(ImpulsesClient));
        ...
        private static DateTime _last_disconnect_server = DateTime.MaxValue;
        /// <summary>
		...
		/// </param>
        public ImpulsesClient(StationRecord[] inp_station_records, string server_address, string tables_path)
        {
...
//			DiagnosticManager.Instance.new_message_impulses_server(string.Format("Загружено {0} станций.", m_data.Stations.Count));
        ...
        private void go()
        {
            ...
            if (m_data.Stations.Count < 1)
			{
               ...
               // System.Windows.MessageBox.Show("Найдено 0 станций. Выход из потока получения данных с сервера.");
                //return;
			}
            ...
            try
            {
                ...
                if (!m_client.IsOpen)
                {
                    try
                    {
                        ...
                        {
                            Connect = true;
                           // LoadProject.Log.Info(
                            if (ConnectDisconnectionServer != null)
                                ConnectDisconnectionServer();
                        }
                        LoadProject.Log.Info(string.Format("Соединение с сервером импульсов {0} установлено", m_connectionString));
                    ...
                    catch
                    {
                        if (_firstconnect)
                        {
                            ...
                            _firstconnect = false;
                            LoadProject.Log.Info(string.Format("Соединение с сервером импульсов {0} потеряно", m_connectionString));
                            if (ConnectDisconnectionServer != null)
                                ConnectDisconnectionServer();
                        }
                        else
...
//					DiagnosticManager.Instance.new_message_impulses_server(string.Format("Ошибка чтения импульсов {0}", e.ToString()));
        ...
        private StatesControl GetStateService(Dictionary<Viewmode, string> impulses, Viewmode state, int station)
        {
            return (impulses.ContainsKey(state)) ? GetImpuls.GetStateControl(station, impulses[state]) : StatesControl.nocontrol;
        }

        /// <summary>
        /// обновляем значения служебных импульсов
        /// </summary>
        private void UpdateValueServiceImpulses()
        {
            try
            {
                foreach (KeyValuePair<int, StationTableServiceTs> station in LoadProject.TsServiceList)
                {
                    foreach (KeyValuePair<string, Dictionary<Viewmode, string>> impulses in station.Value.NamesValue)
                    {
                        StatesControl state_on = GetStateService(impulses.Value, Viewmode.impuls_activ, station.Key);
                        StatesControl state_off = GetStateService(impulses.Value, Viewmode.impuls_pasiv, station.Key);
                        //
                        if ((state_on == StatesControl.activ && state_off != StatesControl.activ) || (state_off == StatesControl.activ && state_on != StatesControl.activ) /*|| (state_on == StatesControl.pasiv && state_off == StatesControl.nocontrol)*/)
                        {
                            if (state_on == StatesControl.activ && state_off != StatesControl.activ)
                                Connections.ClientImpulses.data.Stations[station.Key].TS.set_state(impulses.Key, ImpulseState.ActiveState);
                            else
                                Connections.ClientImpulses.data.Stations[station.Key].TS.set_state(impulses.Key, ImpulseState.PassiveState);
                        }
                        else
...
//			string impulse = impulse_name;
			...
			pCommandRequest->Header.PacketType = (int)Request.Command; // Command type
