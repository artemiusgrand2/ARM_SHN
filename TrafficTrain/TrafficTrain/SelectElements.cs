...
namespace TrafficTrain
{
    ...
    class SelectElements
    {
        ...
        public static BlockSection SelectBlockSection(UIElementCollection collection, Point pointclick)
        {
            foreach (UIElement el in collection)
            {
                ...
                if (_block != null)
                {
                    ...
                    //
                    if (HitStroke(_block.Lines, pointclick, _block, _block.StrokeThickness))
                        return _block;
        ...
        public static bool SelectObjectCommand(UIElementCollection collection, Point pointclick, ref Point Center, ref bool buttonstation)
        {
            //
            if (InfoCommand != null)
            ...
            {
                //
                CommandElement command = el as CommandElement;
                if (command != null)
                {
                    command.CreateCollectionLines();
                    if (HitObject(command.Lines, command.Points, pointclick))
                    {
                        Center.X = command.PointCenter.X;
                        Center.Y = command.PointCenter.Y;
                        //проверяем на наличие комманды в проекте
                        if (Commands.AnalisCommand(command))
                        {
                            //определяем попали на кнопку станции или нет
                            ButtonStation station = el as ButtonStation;
                            if (station != null)
                                buttonstation = true;
                            else buttonstation = false;
                            //
                            return true;
                        }
                    }
        ...
        public static bool MoveCommandButton(UIElementCollection collection, Point pointclick)
        {
            foreach (UIElement el in collection)
            {
                //
                ...
                {
                    command.CreateCollectionLines();
                    if (HitObject(command.Lines, command.Points, pointclick) && el is CommandElement)
                        return true;
                }
            }
            //
        ...
        /// <returns></returns>
        static bool HitStroke(List<Line> _lines, Point pointclick, GraficElement geometry, double strokethickness)
        {
